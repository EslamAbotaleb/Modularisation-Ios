...
final class ArtistDetailsViewModelTests: XCTestCase {
    ...
    @MainActor func test_artist_details_are_not_fetched_again_when_view_appears_again() async {
        // Given
        let expectation = XCTestExpectation()
        artistService.fetchArtistExpectation = expectation
        let sut = makeSUT { _ in
    ...
    @MainActor func test_the_user_is_navigated_when_a_song_is_selected() {
        let expectation = XCTestExpectation()
        let sut = makeSUT { _ in
            expectation.fulfill()
        }
